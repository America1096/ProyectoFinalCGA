...
void Model::loadModel(const std::string & path) {
	...
	temx = sqrt(pow(this->aabb.maxs.x - (float)this->sbb.c.x, 2));
	temy = sqrt(pow(this->aabb.maxs.y - (float)this->sbb.c.y, 2));
	temz = sqrt(pow(this->aabb.maxs.z - (float)this->sbb.c.z, 2));

	if ((temx <= temy && temx >= temz) or (temx >= temy && temx <= temz) ) {
		this->sbb.ratio = temx + temy;
	}
	else if ((temy <= temx && temy >= temz) or (temy >= temx && temy <= temz)) {
		this->sbb.ratio = temy + temz;
	}
	else if ((temz <= temx && temz >= temy) or (temz >= temx && temz <= temy) ){
		this->sbb.ratio = temz + temx;
	}
}
