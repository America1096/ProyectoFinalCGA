...
// Posiciones de coin
std::vector<glm::vec3> coinPosition = { glm::vec3(35.16, 0, -38.67), glm::vec3(46.09, 0, -83.98),
glm::vec3(85.16, 0, -49.22), glm::vec3(80.86, 0, 6.25), glm::vec3(30.47, 0, 17.97),
glm::vec3(-4.69, 0, -14.06), glm::vec3(-60.94, 0, -0.78), glm::vec3(-52.73, 0, 37.11),
glm::vec3(-27.34, 0, 64.45), glm::vec3(60.16, 0, 48.44), glm::vec3(56.25, 0, 88.67),
glm::vec3(-6.25, 0, 89.45), glm::vec3(-54.69, 0, 88.28), glm::vec3(-87.11, 0, 74.22),
glm::vec3(-87.11, 0, 11.33), glm::vec3(-87.11, 0, -47.66) };
...
// Posiciones de gemas
std::vector<glm::vec3> gemaPosition = { glm::vec3(75.39, 0, -81.25), glm::vec3(60.94, 0, 10.94),
glm::vec3(-32.81, 0, -16.8), glm::vec3(17.58, 0, 51.56), glm::vec3(21.09, 0, 85.94),
glm::vec3(-89.84, 0, 46.48), glm::vec3(-88.28, 0, -28.12) };
...
Terrain terrain(-1, -1, 200, 8, "../Textures/testHeightProyecto4.png");
...
glm::mat4 modelMatrixTren = glm::mat4(1.0f);
glm::mat4 modelMatrixTrack1 = glm::mat4(1.0f);

// Model matrix bardas
glm::mat4 modelMatrixBarda2 = glm::mat4(1.0f);
glm::mat4 modelMatrixBarda3 = glm::mat4(1.0f);
glm::mat4 modelMatrixBarda5 = glm::mat4(1.0f);
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	//Tren 
	trenModel.loadModel("../models/Proyecto/Tren/ID02664_00000000out.obj");
	trenModel.setShader(&shaderMulLighting);
	//Track1
	track1Model.loadModel("../models/Proyecto/ChaoRace/track8.obj");
	track1Model.setShader(&shaderMulLighting);
	//Track4
//	track4Model.loadModel("../models/Proyecto/Track And The Beanstalk/track7.obj");
//	track4Model.setShader(&shaderMulLighting);

	//Bardas
	barda2Model.loadModel("../models/Proyecto/Bardas/Destroid Walls/DestroyableWalls.obj");
	barda2Model.setShader(&shaderMulLighting);

	barda3Model.loadModel("../models/Proyecto/Bardas/Military-Fence-Gate/militaryFenceGate.obj");
	barda3Model.setShader(&shaderMulLighting);

	muro1Model.loadModel("../models/Proyecto/Bardas/Wall/Models and Textures/wall.obj");
	muro1Model.setShader(&shaderMulLighting);

	//muro2Model.loadModel("../models/Proyecto/Bardas/Wall/Models and Textures/wall4.obj");
	//muro2Model.setShader(&shaderMulLighting);

	barda5Model.loadModel("../models/Proyecto/Bardas/WarpStone Warp Tunnel/WarpStone WarpTunnel.obj");
	barda5Model.setShader(&shaderMulLighting);
	...
	// Definiendo la textura a utilizar
	Texture textureTerrainBlendMap("../Textures/blendMapProyecto6.png");
	// Carga el mapa de bits (FIBITMAP es el tipo de dato de la libreria)
...
//	buffer[2] = alutCreateBufferFromFile("../sounds/darth_vader.wav");
	buffer[2] = alutCreateBufferFromFile("../sounds/car_acel.wav");
...
/*	alSourcef(source[3], AL_PITCH, 1.0f);
	alSourcef(source[3], AL_GAIN, 1.0f); //Ganacia, que tan fuerte se escucha
	...
	alSourcei(source[3], AL_LOOPING, AL_TRUE); //Una vez que ya se reprodujo una vez el sonido volverlo hacer o no
...
/*		if (modelSelected == 1)
	...
	//Lambo animation
	if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_N) == GLFW_PRESS && nitro >= 10) {
		avanza = true;
		velocidad = 1.0;
		usarNitro = true;
		frenado1 = false;
	...
	if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS && avanza) {
		std::cout << "Avanza Select1 " << avanza << std::endl;
	...
	else if (modelSelected == 1 && glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS && avanza) {
		std::cout << "Avanza Select2 " << avanza << std::endl;
		modelMatrixLambo = glm::translate(modelMatrixLambo, glm::vec3(0.83827, -0.17224, -1.47627));
		modelMatrixLambo = glm::rotate(modelMatrixLambo, -glm::radians(1.0f), glm::vec3(0, 1, 0));
		modelMatrixLambo = glm::translate(modelMatrixLambo, glm::vec3(-0.83827, 0.17224, 1.47627));
	}
...
void applicationLoop() {
	...
	modelMatrixLambo = glm::translate(modelMatrixLambo, glm::vec3(-44.14, 0.0, -50.78));
	...
	modelMatrixFabrica = glm::translate(modelMatrixFabrica, glm::vec3(-83.59f, 0.0f, -82.03f));
	modelMatrixFabrica = glm::rotate(modelMatrixFabrica, glm::radians(-90.0f), glm::vec3(0, 1, 0));

	modelMatrixGym = glm::translate(modelMatrixGym, glm::vec3(-57.89f, 0.0f, -82.03f));
	modelMatrixGym = glm::rotate(modelMatrixGym, glm::radians(-90.0f), glm::vec3(0, 1, 0));

	modelMatrixHouse = glm::translate(modelMatrixHouse, glm::vec3(-26.19f, 0.0f, -82.03f));
	modelMatrixHouse = glm::rotate(modelMatrixHouse, glm::radians(-90.0f), glm::vec3(0, 1, 0));

	modelMatrixTren = glm::translate(modelMatrixTren, glm::vec3(20.0f, 0.0f, 50.0f));

	modelMatrixTrack1 = glm::translate(modelMatrixTrack1, glm::vec3(-0.08, 0, 16.95));

	//posicion para bardas
	modelMatrixBarda2 = glm::translate(modelMatrixBarda2, glm::vec3(0.0f, 0.0f, -20.0f));

	modelMatrixBarda3 = glm::translate(modelMatrixBarda3, glm::vec3(0.0f, 0.0f, -30.0f));

	modelMatrixMuro1 = glm::translate(modelMatrixMuro1, glm::vec3(-99.0, 0, 0.0));
	modelMatrixMuro1 = glm::rotate(modelMatrixMuro1, glm::radians(90.0f), glm::vec3(0, 1, 0));

	//modelMatrixMuro2 = glm::translate(modelMatrixMuro2, glm::vec3(0.0, 0, 99.0));

	modelMatrixBarda5 = glm::translate(modelMatrixBarda5, glm::vec3(10.0f, 0.0f, 0.0f));
	...
	*/
	...
	while (psi) {
		...
		*/
		...
		mayowCollider.c = glm::vec3(modelMatrixColliderMayow[3]);
		mayowCollider.ratio = mayowModelAnimate.getCbb().ratio * 0.021 * 0.787401574;
		...
		for (std::map<std::string, std::tuple<AbstractModel::CBB, glm::mat4, glm::mat4> >::iterator it =
			...
			glm::mat4 matrixCollider = glm::mat4(1.0);
			matrixCollider = glm::translate(matrixCollider, std::get<0>(it->second).c);
			matrixCollider = glm::scale(matrixCollider, glm::vec3(std::get<0>(it->second).ratio));
			matrixCollider = glm::scale(matrixCollider, glm::vec3(std::get<0>(it->second).heigth));
			cylinderCollider.setColor(glm::vec4(1.0, 1.0, 1.0, 1.0));
...
void prepareDepthScene() {
	...
	//Tren 
	trenModel.setShader(&shaderDepth);
	//Track1
	track1Model.setShader(&shaderDepth);
	//Track4
//	track4Model.setShader(&shaderDepth);
	//Bardas
	barda2Model.setShader(&shaderDepth);
	barda3Model.setShader(&shaderDepth);
	muro1Model.setShader(&shaderDepth);
	//muro2Model.setShader(&shaderDepth);
	barda5Model.setShader(&shaderDepth);
...
/*	matrixModelRock[3][1] = terrain.getHeightTerrain(matrixModelRock[3][0], matrixModelRock[3][2]);
	...
	modelMatrixTren[3][1] = terrain.getHeightTerrain(modelMatrixTren[3][0], modelMatrixTren[3][2]);
	glm::mat4 modelMatrixTrenBody = glm::mat4(modelMatrixTren);
	//modelMatrixTrenBody = glm::scale(modelMatrixTrenBody, glm::vec3(2.0, 2.0, 2.0));
	trenModel.render(modelMatrixTrenBody);
	...
	modelMatrixTrack1Body = glm::scale(modelMatrixTrack1Body, glm::vec3(0.5, 0.5, 0.5));
	track1Model.render(modelMatrixTrack1Body);

	//Renderizado de bardas
	modelMatrixBarda2[3][1] = terrain.getHeightTerrain(modelMatrixBarda2[3][0], modelMatrixBarda2[3][2]);
	glm::mat4 modelMatrixBarda2Body = glm::mat4(modelMatrixBarda2);
	//		modelMatrixBarda2Body = glm::scale(modelMatrixBarda2Body, glm::vec3(1.0, 0.5, 1.0));
	//barda2Model.render(modelMatrixBarda2Body);

	modelMatrixBarda3[3][1] = terrain.getHeightTerrain(modelMatrixBarda3[3][0], modelMatrixBarda3[3][2]);
	glm::mat4 modelMatrixBarda3Body = glm::mat4(modelMatrixBarda3);
	//		modelMatrixBarda3Body = glm::scale(modelMatrixBarda3Body, glm::vec3(1.0, 0.5, 1.0));
	//barda3Model.render(modelMatrixBarda3Body);
	...
	modelMatrixMuro1Body = glm::scale(modelMatrixMuro1Body, glm::vec3(17.5, 2.0, 1.0));
	...
	//tunel
	modelMatrixBarda5[3][1] = terrain.getHeightTerrain(modelMatrixBarda5[3][0], modelMatrixBarda5[3][2]);
	glm::mat4 modelMatrixBarda5Body = glm::mat4(modelMatrixBarda5);
	//		modelMatrixBarda5Body = glm::scale(modelMatrixBarda5Body, glm::vec3(1.0, 0.5, 1.0));
	//barda5Model.render(modelMatrixBarda5Body);
