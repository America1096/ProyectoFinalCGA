...
//Shader para las particulas del nitro
...
//Posicion del topito
glm::vec3 posicionTopito = glm::vec3(0.0f, 0.0f, -90.0f);
...
//Variable para escalar cajas de colision
float escalaColision = 0.08f;
bool enableEscala = true;
bool enableEscala2 = true;
...
std::map<std::string, glm::vec3> blendingUnsorted = {
		...
		{"topito", posicionTopito},
		{"nitro", glm::vec3(-35.12, 0.0, -62.0)},
};
...
//Particulas para el nitro
GLuint initVelNitro, startTimeNitro;
GLuint VAOParticlesNitro;
GLuint nParticlesNitro = 10000;
double currTimeParticlesAnimationNitro, lastTimeParticlesAnimationNitro;

void initParticleBuffersNitro() {
	// Generate the buffers
	glGenBuffers(1, &initVelNitro);   // Initial velocity buffer
	glGenBuffers(1, &startTimeNitro); // Start time buffer

	// Allocate space for all buffers
	int size = nParticlesNitro * 3 * sizeof(float);
	glBindBuffer(GL_ARRAY_BUFFER, initVelNitro);
	glBufferData(GL_ARRAY_BUFFER, size, NULL, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, startTimeNitro);
	glBufferData(GL_ARRAY_BUFFER, nParticlesNitro * sizeof(float), NULL, GL_STATIC_DRAW);

	// Fill the first velocity buffer with random velocities
	glm::vec3 v(0.0f);
	float velocity, theta, phi;
	GLfloat *data = new GLfloat[nParticlesNitro * 3];
	...
	// Fill the start time buffer
	delete[] data;
	data = new GLfloat[nParticlesNitro];
	float time = 0.0f;
	float rate = 0.0009f;
	for (unsigned int i = 0; i < nParticlesNitro; i++) {
		data[i] = time;
	...
	glBindBuffer(GL_ARRAY_BUFFER, startTimeNitro);
	glBufferSubData(GL_ARRAY_BUFFER, 0, nParticlesNitro * sizeof(float), data);

	glBindBuffer(GL_ARRAY_BUFFER, 0);
	delete[] data;

	glGenVertexArrays(1, &VAOParticlesNitro);
	glBindVertexArray(VAOParticlesNitro);
	glBindBuffer(GL_ARRAY_BUFFER, initVelNitro);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, NULL);
	glEnableVertexAttribArray(0);
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	shaderParticlesNitro.initialize("../Shaders/particlesFountain.vs", "../Shaders/particlesFountain.fs");
...
//	glBindTexture(GL_TEXTURE_2D, textureWindowID);
	...
	//	glUniformMatrix4fv(shaderText.getUniformLocation("projection"), 1, GL_FALSE, glm::value_ptr(projection));
	...
	// Definiendo la textura a utilizar
	Texture textureWindow("../Textures/pantallaInicio.PNG");
	// Carga el mapa de bits (FIBITMAP es el tipo de dato de la libreria)
...
// Definiendo la textura a utilizar
	...
	//Particulas nitro
	Texture textureParticlesNitro("../Textures/Op1.png");
	bitmap = textureParticlesNitro.loadImage();
	data = textureParticlesNitro.convertToData(bitmap, imageWidth, imageHeight);
	...
	if (data) {
		...
		std::cout << "Failed to load texture" << std::endl;
	textureParticlesNitro.freeImage(bitmap);
	...
	std::uniform_real_distribution<float> distr01 = std::uniform_real_distribution<float>(0.0f, 0.5f); //Ancho particula fuego
	...
	*******************************************/
	...
	alSourcef(source[4], AL_PITCH, 1.0f); //Que tan rapido se reproduce el sonido
	alSourcef(source[4], AL_GAIN, 3.0f); //Ganacia, que tan fuerte se escucha
	alSourcefv(source[4], AL_POSITION, source4Pos); //Posicion
	alSourcefv(source[4], AL_VELOCITY, source4Vel); //Velocidad
	alSourcei(source[4], AL_BUFFER, buffer[4]); //Buffer asociado a esa fuente
	alSourcei(source[4], AL_LOOPING, AL_FALSE); //Una vez que ya se reprodujo una vez el sonido volverlo hacer o no
	alSourcef(source[4], AL_MAX_DISTANCE, 500); //Umbral para saber cuanto se tiene que alejar para que se escuche menos el sonido
...
void destroy() {
	...
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	glDeleteBuffers(1, &initVelNitro);
	glDeleteBuffers(1, &startTimeNitro);
	glBindVertexArray(0);
...
bool processInput(bool continueApplication) {
	...
	if (enableEscala && glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS) {
		enableEscala = false;
		escalaColision += 0.01;
		printf("Escala del modelo: %f", escalaColision);
	}
	else if (glfwGetKey(window, GLFW_KEY_2) == GLFW_RELEASE)
		enableEscala = true;

	if (enableEscala2 && glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS) {
		enableEscala2 = false;
		escalaColision -= 0.01;
		printf("Escala del modelo: %f", escalaColision);
	}
	...
	if (glfwJoystickPresent(GLFW_JOYSTICK_1)) {
		...
		if (modelSelected == 1 && (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) && vida > 0 && iniciaCarrera && ganar == false) {
			...
			/*glm::vec3 VectorTerreno = terrain.getNormalTerrain(modelMatrixLambo[3].x - 0.83827, modelMatrixLambo[3].z + 1.47627);
			glm::vec3 Cruz = normalize(glm::cross(VectorRotacion, VectorTerreno));

			printf("\nVector terreno: %f, %f, %f\n", VectorTerreno[0], VectorTerreno[1], VectorTerreno[2]);
			printf("\nVector de rotacion: %f, %f, %f\n", modelMatrixLambo[2].x, modelMatrixLambo[2].y, modelMatrixLambo[2].z);
			float AnguloP = Angulo(VectorRotacion, VectorTerreno);
			printf("\n Angulo: %f", AnguloP);

			modelMatrixLambo = glm::rotate(modelMatrixLambo, -AnguloAnterior, glm::vec3(1, 0, 0));
			modelMatrixLambo = glm::rotate(modelMatrixLambo, AnguloP, glm::vec3(1, 0, 0));
			AnguloAnterior = AnguloP;
			*/
		}
...
void applicationLoop() {
	...
	//modelMatrixGanar = glm::rotate(modelMatrixGanar, glm::radians(-90.0f), glm::vec3(0, 1, 0));

	modelMatrixPerder = glm::translate(modelMatrixPerder, glm::vec3(-28.12, 0, -64.84));
	//	modelMatrixPerder = glm::rotate(modelMatrixPerder, glm::radians(-90.0f), glm::vec3(0, 1, 0));
	...
	//Topo
	modelMatrixTopito = glm::translate(modelMatrixTopito, posicionTopito);
	...
	while (psi) {
		...
		*///Se crea matriz de forma manual la caja para la matriz de proyeccion
		...
		shaderParticlesNitro.setMatrix4("projection", 1, false,
			glm::value_ptr(projection));
		shaderParticlesNitro.setMatrix4("view", 1, false,
			glm::value_ptr(view));
		...
		*******************************************/
		...
		// Collider de track1
		AbstractModel::CBB Track1Collider;
		glm::mat4 modelMatrixColliderTrack1 = glm::mat4(modelMatrixTrack1);
		modelMatrixColliderTrack1 = glm::rotate(modelMatrixColliderTrack1,
			glm::radians(-90.0f), glm::vec3(1, 0, 0));
		modelMatrixColliderTrack1 = glm::scale(modelMatrixColliderTrack1, glm::vec3(1.5, 1.5, 1.5));
		modelMatrixColliderTrack1 = glm::translate(modelMatrixColliderTrack1, track1Model.getCbb().c);
		Track1Collider.c = glm::vec3(modelMatrixColliderTrack1[3]);
		Track1Collider.ratio = mayowModelAnimate.getCbb().ratio*0.12;
		Track1Collider.heigth = mayowModelAnimate.getCbb().heigth*0.08;
		addOrUpdateColliders(collidersCBB, "track1", Track1Collider, modelMatrixTrack1);
		...
		mayowCollider.ratio = mayowModelAnimate.getCbb().ratio * 0.021 * 0.347;
		mayowCollider.heigth = mayowModelAnimate.getCbb().heigth * 0.021 * 0.787401574;
		...
		/*else
		{
			supermanCollider.c = glm::vec3(-30.0f, -100.0f, 0);
			supermanCollider.e = glm::vec3(0.0f, 0.0f, 0);
			supermanCollider.e = glm::vec3(0.0f, 0.0f, 0);
			addOrUpdateColliders(collidersOBB, "superman", supermanCollider, modelMatrixSuperman);
		}*/
		...
		modelMatrixColliderTopito = glm::scale(modelMatrixColliderTopito, glm::vec3(0.34, 1.0, 1.0));
		modelMatrixColliderTopito = glm::translate(modelMatrixColliderTopito, muro1Model.getObb().c);
		TopitoCollider.e = muro1Model.getObb().e*glm::vec3(0.34, 1.0, 1.0);
		TopitoCollider.c = glm::vec3(modelMatrixColliderTopito[3]);
		addOrUpdateColliders(collidersOBB, "topito", TopitoCollider, modelMatrixTopito);
				...
				if (it != jt
					...
					if (it->first == "lambo" || jt->first == "lambo")
					{
						if (it->first != "arco" && jt->first != "arco")
						{
							...
							if (vida <= 0) {
								std::cout << "Lambo x " << modelMatrixLambo[3].x << std::endl;
								std::cout << "Lambo y " << modelMatrixLambo[3].y << std::endl;
								std::cout << "Lambo z " << modelMatrixLambo[3].z << std::endl;
								std::cout << "Lambo rx " << modelMatrixLambo[2].x << std::endl;
								std::cout << "Lambo ry " << modelMatrixLambo[2].y << std::endl;
								std::cout << "Lambo rz " << modelMatrixLambo[2].z << std::endl;
						...
						{
							std::cout << "Se detecto arco" << std::endl;
							detectarArco = true;
						}
					}
					isCollision = true;
				...
				if (testSphereOBox(std::get<0>(it->second),
					...
					for (int i = 0; i < TAMCOIN; i++)
					{
						if (it->first == coinValor[i] && jt->first == "lambo")
						{
							...
							//std::cout << "sonidoCoin: " << sonidoCoin << std::endl;
						}
					}
					for (int i = 0; i < TAMGEMA; i++)
					{
		...
		for (colIt = collisionDetection.begin(); colIt != collisionDetection.end();
			...
			if (kt != collidersCBB.end()) {
				...
				//if (kt->first.compare("track1") == 0)
					//	modelMatrixTrack1 = std::get<1>(kt->second);
		...
		//Nitro del auto
		if (contadorNitro <= 1 && usarNitro == true) {
			avanza = true;
			frenado1 = false;
			contadorNitro += 0.05;
			velocidad = 1.0;
			...
			//			std::cout << "frenado1 " << frenado1 << std::endl;
		...
		*****************************/
		...
		if (presionaTecla && creditos == false && modelSelected == 1)
		{
			...
			if (ganar == false) {
				...
				******************/
				...
				/*if (iniciaCarrera == false) //Cuenta 3 segundos para que inicie la carrera
				{
					timecount += 1;
					...
					RenderText(shaderText, std::to_string(segundos), 400.0f, 350.0f, 3.0f, glm::vec3(0.5, 0.8f, 0.2f));
					if (segundos == 3) {
						iniciaCarrera = true;
						timecount = 1;
						segundos = 20;
		...
		if (presionaTecla)
		{
			...
			{
				//std::cout << "avanza: " << avanza << std::endl;
				//std::cout << "retrocede: " << retrocede << std::endl;

				sourcesPlay[2] = false;
				alSourcePlay(source[2]);
			}
			else if (avanza == false && retrocede == false) {
				//std::cout << "avanza detener sonido: " << avanza << std::endl;
				//std::cout << "retrocede: " << retrocede << std::endl;
...
//	camera->getViewMatrix();
	...
	if (creditos)
	{
		...
		//boxFinal.setOrientation(glm::vec3(0, 0, -30));
	...
	for (std::map<float, std::pair<std::string, glm::vec3> >::reverse_iterator it = blendingSorted.rbegin(); it != blendingSorted.rend(); it++) {
		...
		if (it->second.first.compare("lambo") == 0 /*&& contVida<0.5*/) {
			// Lambo car
			...
			if (region <= 0 || region2 <= 0 || region3 <= 0)
			{
				std::cout << "region: " << region << ", velocidad: " << velocidad << std::endl;
				std::cout << "region2: " << region2 << ", velocidad: " << velocidad << std::endl;
				std::cout << "region3: " << region3 << ", velocidad: " << velocidad << std::endl;
			...
			modelMatrixLambo = glm::translate(modelMatrixLambo, glm::vec3(-0.03827, 0.17224, 1.47627));
			...
			glm::mat4 modelMatrixLamboLlantasIzqEnf = glm::mat4(modelMatrixLamboChasis);
			//Izquierda enfrente
			modelMatrixLamboLlantasIzqEnf = glm::translate(modelMatrixLamboLlantasIzqEnf, glm::vec3(-0.83827, 0.17224, 1.47627));
			modelMatrixLamboLlantasIzqEnf = glm::rotate(modelMatrixLamboLlantasIzqEnf, velocidad, glm::vec3(1, 0, 0));
			modelMatrixLamboLlantasIzqEnf = glm::translate(modelMatrixLamboLlantasIzqEnf, glm::vec3(0.83827, -0.17224, -1.47627));

			glm::mat4 modelMatrixLamboLlantasDerEnf = glm::mat4(modelMatrixLamboChasis);
			//Derecha enfrente
			modelMatrixLamboLlantasDerEnf = glm::translate(modelMatrixLamboLlantasDerEnf, glm::vec3(0.83827, -0.17224, 1.47627));
			modelMatrixLamboLlantasDerEnf = glm::rotate(modelMatrixLamboLlantasDerEnf, velocidad, glm::vec3(1, 0, 0));
			modelMatrixLamboLlantasDerEnf = glm::translate(modelMatrixLamboLlantasDerEnf, glm::vec3(-0.83827, 0.17224, -1.47627));

			glm::mat4 modelMatrixLamboLlantaIzqAtras = glm::mat4(modelMatrixLamboChasis);
			//Izquierda Atras
			modelMatrixLamboLlantaIzqAtras = glm::translate(modelMatrixLamboLlantaIzqAtras, glm::vec3(-0.83827, 0.17224, -1.47627));
			modelMatrixLamboLlantaIzqAtras = glm::rotate(modelMatrixLamboLlantaIzqAtras, velocidad, glm::vec3(1, 0, 0));
			modelMatrixLamboLlantaIzqAtras = glm::translate(modelMatrixLamboLlantaIzqAtras, glm::vec3(0.83827, -0.17224, 1.47627));

			glm::mat4 modelMatrixLamboLlantasDerAtras = glm::mat4(modelMatrixLamboChasis);
			//Derecha enfrente
			modelMatrixLamboLlantasDerAtras = glm::translate(modelMatrixLamboLlantasDerAtras, glm::vec3(0.83827, -0.17224, -1.47627));
			modelMatrixLamboLlantasDerAtras = glm::rotate(modelMatrixLamboLlantasDerAtras, velocidad, glm::vec3(1, 0, 0));
			modelMatrixLamboLlantasDerAtras = glm::translate(modelMatrixLamboLlantasDerAtras, glm::vec3(-0.83827, 0.17224, 1.47627));

			modelLamboLeftDor.render(modelMatrixLamboChasis);
			modelLamboRightDor.render(modelMatrixLamboChasis);
			modelLamboFrontLeftWheel.render(modelMatrixLamboLlantasIzqEnf);
			modelLamboFrontRightWheel.render(modelMatrixLamboLlantasDerEnf);
			modelLamboRearLeftWheel.render(modelMatrixLamboLlantaIzqAtras);
			modelLamboRearRightWheel.render(modelMatrixLamboLlantasDerAtras);
		...
		else if (renderParticles && it->second.first.compare("nitro") == 0 /*&& ActivaTopito == 0*/) {
		glm::mat4 modelMatrixParticlesNitro = glm::mat4(1.0);
		modelMatrixParticlesNitro = glm::translate(modelMatrixParticlesNitro, it->second.second);
		modelMatrixParticlesNitro[3][1] = terrain.getHeightTerrain(modelMatrixParticlesNitro[3][0], modelMatrixParticlesNitro[3][2]);
		modelMatrixParticlesNitro = glm::rotate(modelMatrixParticlesNitro, glm::radians(90.0f), glm::vec3(0.0, 0.0, 1.0));

		modelMatrixParticlesNitro = glm::scale(modelMatrixParticlesNitro, glm::vec3(10.0, 10.0, 10.0));
		currTimeParticlesAnimationNitro = TimeManager::Instance().GetTime();
		if (currTimeParticlesAnimationNitro - lastTimeParticlesAnimationNitro > 10.0) {
...
//			ActivaTopito = 1;
		...
		shaderParticlesNitro.setFloat("Time", float(currTimeParticlesAnimationNitro - lastTimeParticlesAnimationNitro));
		shaderParticlesNitro.setFloat("ParticleLifetime", 3.5f);
		shaderParticlesNitro.setInt("ParticleTex", 0);
		shaderParticlesNitro.setVectorFloat3("Gravity", glm::value_ptr(glm::vec3(-0.3f, 0.0f, 0.0f)));
		shaderParticlesNitro.setMatrix4("model", 1, false, glm::value_ptr(modelMatrixParticlesNitro));
		glBindVertexArray(VAOParticlesNitro);
